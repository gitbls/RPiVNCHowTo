...
**NOTE:** This can be used on other Linux distros with a recent systemd implementation, although minor adjustments may be required if the distro is not Debian-based. If you run into problems, please let me know!
...
But, if you'd prefer to not use sdm, that's fine. RealVNC Server (which attaches to the Pi4 graphical console) works great with TigerVNC-powered virtual desktops installed using the technique on this github. `make-systemd-xvnc` and this documentation skip port 5900 for RealVNC compatibility.
...
* **TigerVNC server:** `sudo apt-get install tigervnc-standalone-server xfonts-scalable xfonts-100dpi xfonts-75dpi --no-install-recommends`
* **TightVNC server:** `sudo apt-get install tightvncserver xfonts-scalable xfonts-100dpi xfonts-75dpi --no-install-recommends`
...
RasPiOS Lite is exactly that...Lite. You'll need to install a few more packages to use these dynamic virtual desktops.
...
* You'll also need a *Window Manager*. I prefer icewm, but you might | probably will prefer something different. In any case, you'll need to `sudo apt-get install` your Window Manager of choice.

## System Configuration

This section describes in more detail the system configuration changes required to enable virtual VNC desktops. If you follow the directions for RasPiOS Full, you do not need to do any of this if you've performed the steps described above for *Installing onto RasPiOS Full*.
...
`sudoedit /etc/lightdm/lightdm.conf` and modify the XDMCPServer section as follows
...
After you've completed this change, `sudo systemctl restart lightdm.service` to restart the service with the new settings (this will log out your session!), or restart your system. If you don't, you'll only see a black screen when you connect to the system with a VNC viewer.

If you prefer to not have lightdm to run on the (HDMI) console, that is, you only want a command-line console, `sudoedit /etc/lightdm/lightdm.conf` again and after the line `#start-default-seat=true` add this new line.
```
...
xdm is a lightweight Display Manager with less capabilities than lightdm. That said, I've found it to consume less system resources than lightdm, and can be coupled with choosewm (see Appendix below).
...
**Note**: If your system has both lightdm and xdm installed you can switch between the two display managers, using `dpkg-reconfigure lightdm`.

### VNC Service configuration

**NOTE** If you aren't comfortable and experienced at editing system configuration files, I strongly encourage you to use the `make-systemd-xvnc` script on this github.

The following explanation provides background on how to edit the configuration files, but these are not exhaustive and step-by-step.
...
* **Test** that you can use a VNC client to connect to your server. If you get a black screen in the VNC window make sure that you have restarted the display manager, or restart the system.
...
The VNC protocol by itself is insecure, so you shouldn't use it over insecure networks, such as the internet, without taking precautions. With the solution documented here you need to use a VPN or similar solution (ssh, meet-in-the middle, etc) to ensure a secure connection. As an aside, presumably that RealVNC's Cloud Connections use an enhanced VNC protocol to ensure high security.

## How does this work?

Once the xvnc1.socket is started, systemd listens on the specified TCP port. When a network connect lands on that port, systemd starts the service described by the .service file with the filename corresponding to the socket name. This is quite simple and elegant, and it *just works*.

In the event that it doesn't just work, see the next section. LOL.
...
Things that I always look at when something doesn't work correctly:

* If a **VNC viewer window opens but it's black**, it's a display manager problem.
    * Did you reboot the system before trying to connect?
    * Make sure that you've configured the display manager (lightdm or xdm) correctly, especially if you edited the config files manually rather than using the scripts from this github.
* If the **VNC viewer cannot connect to the server** it's an xvnc socket/service pair problem.
    * Is systemd listening to the assigned ports? Use `sudo netstat -ltpn | grep 59` to confirm that ports 5901, 5902, ... are listed. If not, there is something wrong with your xvnc socket/service pairs
    * Make sure that the socket/service pairs exist and are correct, as expected
 
## Reporting problems or difficulties

If you are unable to resolve your issue, download and run `xvnc-collect` from this github, and provide the output from it (/tmp/rpivnchowto-diagnostics) with your Issue that you open on this github.
...
exec /usr/bin/xterm
```
Note that if there is only have one window manager installed, it will be run automatically after login.